		...
		while (b) {
			...
			try {
				...
				case 1: {
					...
					Rail r = new Rail(starting, destination, leaving, arrival, numOdMidStations, midStationName,
							midStationTime);
					r.save(pw);
				...
				case 3:
					...
					for (int i = 0; i < tk.getTripNum(); i++) {
						...
						for (int j = 0; j < tk.getRails()[i].getMidStationsNum(); j++) {
							if (starting.equals(tk.getRails()[i].getMidStationsNames()[j]) && upTo3Leaving[i] == null) {
								if (tk.timeCheck(leaving, tk.getRails()[i].getMidStationsTimes()[j]) == true) {
					...
					for (int i = 0; i < 3; i++) {
						if (upTo3Leaving[i] != null && upTo3Arrive[i] != null) {
							System.out.println("Leaving from " + starting + "at " + upTo3Leaving[i] + " , arriving to"
									+ destination + "at " + upTo3Arrive[i] + "\n");
						}
					}
				...
				System.out.println("Not available option -- (not exist) , try again\n");
			} catch (Exception e) {
